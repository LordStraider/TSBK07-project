...
void home()
{
    ...
    //CFURLRef homeURL = CFBundleCopyResourcesDirectoryURL(mainBundle);
	CFURLRef homeURL = CFBundleCopyBundleURL(mainBundle);
    //CFURLRef homeURL = CFURLCreateCopyDeletingLastPathComponent(kCFAllocatorDefault, bundleURL);

	char path[PATH_MAX];
	if (!CFURLGetFileSystemRepresentation(homeURL, TRUE, (UInt8 *)path, PATH_MAX))
	{
		// error!
		return;
	}
